...
def main():
    model_name = 'Drought'
    dataset_path = r'Method 2\Data\training_data_Drought.csv'
    ...
    # Probability of any drought (Moderate + Severe + Extreme) = classes 1, 2, 3
    # day1_probs[:, 1:] sums the columns for Moderate(1), Severe(2), Extreme(3)
    drought_prob = np.sum(day1_probs[:, 1:], axis=1)
    
    # Assign rounded probability back to dataframe
    df['drought_probability'] = np.nan
    df.loc[predict_mask, 'drought_probability'] = np.round(drought_prob, 2)
    
    print("Saving updated dataset...")
    df.to_csv(os.path.join(base_dir, dataset_path), index=False)
    print("Done! Appended 'drought_probability' (0.00 to 1.00)")
