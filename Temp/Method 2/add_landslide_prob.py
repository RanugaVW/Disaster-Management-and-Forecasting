...
def main():
    ...
    dataset_path = r'Method 2\Data\training_data_Landslide.csv'
    ...
    # Probability of any landslide (Moderate + Severe + Extreme) = classes 1, 2, 3
    # day1_probs[:, 1:] sums the columns for Moderate(1), Severe(2), Extreme(3)
    landslide_prob = np.sum(day1_probs[:, 1:], axis=1)
    
    # Assign rounded probability back to dataframe
    df['landslide_probability'] = np.nan
    df.loc[predict_mask, 'landslide_probability'] = np.round(landslide_prob, 2)
    
    print("Saving updated dataset...")
    df.to_csv(os.path.join(base_dir, dataset_path), index=False)
    print("Done! Appended 'landslide_probability' (0.00 to 1.00)")
